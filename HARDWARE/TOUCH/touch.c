...
{					  
	...
	*/
	...
	SPI_Flash_Read(&adj_ok_mark,SAVE_ADDR_BASE+13,1);	
	if(adj_ok_mark==0X0A)//触摸屏已经校准过了			   
	{    												 
...
{	  
	POINT_COLOR=RED;
	LCD_ShowString(40,160,lcddev.width,lcddev.height,16,"WWTx1:");
 	LCD_ShowString(40+80,160,lcddev.width,lcddev.height,16,"WWTy1:");
...
{	
	...
	else
	{
		...
		*/
		...
		GPIOC->CRH|=0X00033000;	//推挽输出 
		//DOUT: PC10
		GPIOC->CRH|=0X00000800;	//推挽输入 
		GPIOC->ODR|=0X1C00;    	//PC10-12 全部上拉  
	
		// DCLK:PD2
		GPIOD->CRL&=0XFFF00F00;
		GPIOD->CRL|=0X00000300;	//推挽输出 
		GPIOD->ODR|=0X0004;    	//PD2上拉 
	
		// /PENIRQ:PA1
		GPIOA->CRL&=0XFFFFFF0F;
		GPIOA->CRL|=0X00000080;	//推挽输出 
		GPIOA->ODR|=0X0002;    	//PA1上拉 
...
#define TOUCH_POS_BUFF_SIZE							3
static POS_T touch_up_pos_buff[TOUCH_POS_BUFF_SIZE];
static u8 touch_cnt = 0;
static u8 no_touch_cnt = 0;
static u8 touch_record_pos = 0;

static void touch_init(void)
{
	int i;
	
	touch_up_pos.x = 0xffff;
	touch_up_pos.y = 0xffff;
	
	for( i=0; i<TOUCH_POS_BUFF_SIZE; i++ )
	{
		touch_up_pos_buff[i].x = 0;
		touch_up_pos_buff[i].y = 0;
	}
...
u8 touch_up_check(void)
{
	...
	{	
		...
		{				
			//保存最近五次按下的位置
			touch_up_pos_buff[touch_record_pos].x = tp_dev.x[0];
			touch_up_pos_buff[touch_record_pos].y = tp_dev.y[0];
			touch_record_pos++;	
			if( touch_record_pos >= TOUCH_POS_BUFF_SIZE )
			{
				touch_record_pos = 0;
			}
			...
			{
				touch_cnt++;
				if( touch_cnt > TOUCH_POS_BUFF_SIZE )
				{
					tmp32 = 0;
					for( i=0; i<TOUCH_POS_BUFF_SIZE; i++ )
					{
						tmp32 += touch_up_pos_buff[i].x;
					}
					touch_up_pos.x = tmp32/TOUCH_POS_BUFF_SIZE;
					
					tmp32 = 0;
					for( i=0; i<TOUCH_POS_BUFF_SIZE; i++ )
					{
						tmp32 += touch_up_pos_buff[i].y;
					}
					touch_up_pos.y = tmp32/TOUCH_POS_BUFF_SIZE;
				}
			}
		}
	}
...
u8 touch_pressed_down(void)
{
	return (( touch_cnt > TOUCH_POS_BUFF_SIZE + 1 ) && (tp_dev.sta&TP_PRES_DOWN));
}
